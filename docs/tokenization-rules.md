...
So the underscore acts as a bridge between the three other kinds of characters you can put in an identifier: `foo_+` is a legal identifier but `foo+` is not; `&_3` is a legal identifier but `&3` is not.

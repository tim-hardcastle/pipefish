...
func TestForLoopCtes(t *testing.T) {
	tests := []test_helper.TestItem{
		...
		{`from a = 0 for i, i = 0, 0; i < 5; i + 1 : a + i`, `comp/for/index/exists.a`},
		...
		{`from a = 0 for a::j = range 5 : a + i`, `comp/for/exists/key.a`},
		{`from a = 0 for i::a = range 5 : a + i`, `comp/for/exists/value.a`},

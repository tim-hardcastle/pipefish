...
func (t SimpleType) describe(mc *vm.Vm) string {
	return mc.DescribeType(values.ValueType(t), vm.LITERAL, 0)
}
...
func (cp Compiler) typeNodeToAlternateType(tn parser.TypeNode) AlternateType {
	...
	case *parser.TypeExpression:
		return cp.typeNodeToAlternateType(cp.P.ToAstType(tn))
	}
...
func (aT AlternateType) ToAbstractType() values.AbstractType {
	...
	return values.AbstractType{result}
}
	...
	for _, u := range aT {
		...
		case SimpleType:
			if u != SimpleType(values.SUCCESSFUL_VALUE) && u != SimpleType(values.ERROR) &&
				u != SimpleType(values.UNSATISFIED_CONDITIONAL) {
				return false
			}
		default:
			return false

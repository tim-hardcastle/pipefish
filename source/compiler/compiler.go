...
NodeTypeSwitch:
	...
	case *parser.AssignmentExpression:
		...
		typeCheckFailed := cp.EmitTypeChecks(rhsResult, node.Right, types, env, newSig, node.Left.GetToken(), flavor)
	...
	case *parser.PeekExpression:
		...
		if b, ok := peeks["o"]; ok && b {
			file, _ := os.Create(filepath.Join(filepath.FromSlash(settings.PipefishHomeDirectory), settings.DUMP_PATH))
			file.Close()
		}
		cp.Vm.PushPeeks(cp.Vm.GetPeeksFromTokens(node.Peeks))
	...
	case *parser.UnfixExpression:
		...
		if node.GetToken().Literal == "*test" {
			cp.Put(vm.Test, resolvingCompiler.Number)
...
func (cp *Compiler) compileForExpression(node *parser.ForExpression, ctxt Context) cpResult {
	...
	if hasBoundVariables {
		...
		boundInitCheck = cp.EmitTypeChecks(
			boundResultLoc,
			node.BoundVariables.(*parser.AssignmentExpression).Right,
			boundVariableTypes,
			newEnv, boundCpSig,
	...
	if flavor == TRIPARTITE {
		...
		indexInitCheck = cp.EmitTypeChecks(
			indexResultLoc,
			node.Initializer.(*parser.AssignmentExpression).Right,
			indexVariableTypes,
			newEnv,
			indexCpSig,
	...
	if hasBoundVariables {
		cp.Cm("Typechecking bound variable result and putting it into bound variables.", tok)
		boundUpdateCheck = cp.EmitTypeChecks(
			boundResultLoc,
			node.Body,
			bodyCpResult.Types,
			newEnv,
	...
	if flavor == TRIPARTITE {
		...
		cp.Cm("Typechecking index variable result and putting it into index variables.", tok)
		indexUpdateCheck = cp.EmitTypeChecks(
			cp.That(),
			node.Update,
			iteratorCpResult.Types,
			newEnv,
			indexCpSig,
			node.Initializer.(*parser.AssignmentExpression).Left.GetToken(),
...
func (cp *Compiler) compileLambda(env *Environment, ctxt Context, fnNode *parser.FuncExpression, tok *token.Token) bool {
	...
	if fnNode.NameRets != nil {
		...
		cp.EmitTypeChecks(
			LF.Model.ResultLocation,
			fnNode.Body,
			bodyCpResult.Types,
			env,
			cp.AstSigToAltSig(rTypes),
			tok,
			CHECK_RETURN_TYPES,
		)
	}
...
func (cp *Compiler) compileOneGivenChunk(node *parser.AssignmentExpression, ctxt *Context) bool {
	...
	for i, pair := range sig {
		...
		if result.Foldable { // TODO --- we should implement constant folding of course, but there's no big hurry because hardly enyone will declare constants here, why would they?
...
func (cp *Compiler) ReserveValidationError(node parser.Node, typename string, valLoc uint32) uint32 {
	cp.Cm("Reserving typeCheckError factory.", node.GetToken())
	err := &vm.ValidationError{Tok: node.GetToken(), Condition: cp.P.PrettyPrint(node),
		Type: typename, Value: valLoc}
	cp.Vm.ValidationErrors = append(cp.Vm.ValidationErrors, err)
	return uint32(len(cp.Vm.ValidationErrors) - 1)
}
...
func (cp *Compiler) EmitTypeChecks(
	...
	expression parser.Node, // The code we compiled to get the loc. Used for generating error messages.
	...
	typeToken *token.Token, // A token saying where the type constraints were declared. Used for error messages.
	flavor typeCheckFlavor, // Says exactly what sort of thing we're tyechecking.
) BkEarlyReturn {
	cp.Cm("Emitting type checks.", typeToken)
	cp.Cm("Sig is "+sig.Describe(cp.Vm)+".", typeToken)
	...
	cp.Cm("Return types are "+types.describe(cp.Vm), typeToken)
	if types.Contains(values.ERROR) {
		cp.Cm("Checking for error.", typeToken)
	...
	if len(singles) == 0 && len(sig) == 1 {
		cp.Throw("comp/typecheck/values/a", typeToken)
		return errorCheck
	}
	if len(tuples) == 0 && len(sig) != 1 {
		cp.Throw("comp/typecheck/values/b", typeToken)
		return errorCheck
	}
	if insert {
		cp.Cm("Inserting into variable", typeToken)
	...
	if len(tuples) == 0 {
		...
		if len(overlap) == 0 || overlap.IsOnly(values.ERROR) {
			cp.Throw("comp/typecheck/type", typeToken)
			return errorCheck
		}
		if len(overlap) != len(singles) {
			cp.Cm("No tuples, sig has length 1, emitting typecheck", typeToken)
			// If the condition is not met, this jumps to where we `ComeFrom` the elements of `typechecks`.
			typeCheck := cp.emitTypeComparisonFromAltType(sig[0].VarType, loc, typeToken)
		...
		cp.Cm("Checking for tuple", typeToken)
		...
		if goodLengths != len(lengths) {
			cp.Cm("Checking tuple lengths", typeToken)
			if lastIsTuple {
		...
		for i := 0; i < len(sig); i++ {
			...
			if elementLoc == DUMMY {
				elementLoc = cp.Reserve(values.UNDEFINED_TYPE, nil, typeToken)
			}
			cp.Emit(vm.IxTn, elementLoc, loc, uint32(i))
			cp.Cm("Emitting type check for "+sig[i].VarType.describe(cp.Vm), typeToken)
			typeCheck := cp.emitTypeComparisonFromAltType(sig[i].VarType, elementLoc, typeToken)
			typeChecks = append(typeChecks, typeCheck)
		}
...
func (cp *Compiler) Emit(opcode vm.Opcode, args ...uint32) {
	if opcode == vm.Rpsh && cp.Rpushes != nil {
		cp.Rpushes = append(cp.Rpushes, uint32(len(cp.Vm.Code)))
	}
	if opcode == vm.Untk && cp.Unthunks != nil {
		cp.Unthunks[uint32(len(cp.Vm.Code))] = uint32(len(cp.Vm.Mem))
	}
...
// Used to comment on the resolution of the multiple dispatch in particular.
func (cp *Compiler) cmP(comment string, tok *token.Token) {
	if settings.PEEK_COMPILER && cp.Vm.IsSet("d") {
		cp.Vm.Dump(comment)
	}
}
...
func (cp *Compiler) CmR(comment string, tok *token.Token) {
	if settings.PEEK_COMPILER {
		cp.Vm.Dump(text.RED + comment + text.RESET)
	}
}
